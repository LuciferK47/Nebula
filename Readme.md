...
Because physical CXL 2.0/3.0 PCIe expansion cards were not available in our test environment, MemTier-MoE provides a software emulation layer:
- **Storage Location**: Hosted in physical system RAM (CPU memory).
- **Bandwidth**: Token-bucket rate limiter calibrated to 8.0 GB/s (or configurable via hardware profiles).
- **Latency**: Calibrated injection (350 ns total access latency) matching CXL Type 3 memory device expectations over PCIe Gen5.
- **Physical Reality**: DRAM and CXL tiers share the same physical host RAM; they are separated logically and differentiated by software delay and rate limiting.

### Multi-Stream CUDA Concurrency
Weight transfers leverage dedicated background CUDA streams (`torch.cuda.Stream`) separated from the default compute stream. CUDA events (`torch.cuda.Event`) coordinate dependency synchronization, enabling non-blocking transfer and compute overlap on supported hardware.
